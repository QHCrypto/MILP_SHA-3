...
//#define width 800
#define width 200
GRBVar x0[width], Y0[width], z0[width], x1[width], y1[width], z1[width], x2[width], y2[width], z2[width], x3[width], y3[width], z3[width], x4[width], y4[width], t0[width / 5][3], t1[width / 5][3], t2[width / 5][3], t3[width / 5][3], t4[width / 5][3];
...
int main()
{
    ...
    try
    {
        ...
        initialize_solution[index(1, 0, 2)] = 1;
        initialize_solution[index(3, 2, 2)] = 1;
        initialize_solution[index(0, 3, 2)] = 1;
        initialize_solution[index(1, 3, 2)] = 1;
        initialize_solution[index(4, 3, 2)] = 1;
        initialize_solution[index(0, 4, 1)] = 1;
        for (int i = 0; i < width; ++i)
			y1[i].set(GRB_DoubleAttr_Start, initialize_solution[i]);
		//for (int i = 0; i < width; ++i)
		//	model.addConstr(y1[i], GRB_EQUAL, initialize_solution[i]);
        printf("y1=\n");
        ...
        for (int i = 0; i < width; ++i)
        {
            ...
            for (auto p : linearlayer[i])
                constr += x4[p];
            model.addConstr(constr, GRB_EQUAL, y4[i]);
        ...
        model.addConstr(k2, GRB_LESS_EQUAL, 12);
		model.setObjective(2 * k3 + 2* k4 + k5, GRB_MINIMIZE);
		model.update();
		model.optimize();
		ofs.open("6rkeccak400.txt");
        ofs << "x1:" << endl;
        log(x1);
		ofs << "y1:" << endl;
		log(y1);
